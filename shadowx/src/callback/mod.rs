...
unsafe fn find_cm_register_callback() -> Result<(*mut u8, *mut u8, *mut u8)> {
    ...
    let insert_call_address = scan_for_pattern(register_callback_internal.cast(), &insert_pattern, 3, 7, 0x108,)?;

    // lea rcx,[nt!CmpCallbackListLock (xxx)]
    let cmp_callback_list_lock_pattern = [0x48, 0x8D, 0x0D];
    let callback_list_lock = scan_for_pattern(insert_call_address.cast(), &cmp_callback_list_lock_pattern, 3, 7, 0x200)?;

    // lea r15,[nt!CallbackListHead (xxx)]
    let callback_list_head_pattern = [0x4C, 0x8D, 0x3D];
    let callback_list_header = scan_for_pattern(insert_call_address.cast(), &callback_list_head_pattern, 3, 7, 0x200)?;

    // lock inc dword ptr [nt!CmpCallBackCount (xxx)]
    let cmp_callback_count_pattern = [0xF0, 0xFF, 0x05];
    let callback_count = scan_for_pattern(insert_call_address.cast(), &cmp_callback_count_pattern, 3, 7, 0x200)?;
